...
contract PositionTokenV2 is Context, IERC20, Ownable, Pausable {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return
            tokenFromReflection(_rOwned[account]).add(genesisBalance(account));
    }
    ...
    function notifyGenesisAddresses(address[] memory _receivers, uint256 _value)
        public
        onlyOwner
    {
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _receivers.length; i++) {
            _rOwned[_receivers[i]] = _rOwned[_receivers[i]].add(
                _value.mul(_currentRate)
            );
            emit Transfer(address(0), _receivers[i], _value);
        }
    }
    ...
    function _transferStandard(
        ...
        _rOwned[recipient] = _rOwned[recipient].add(
            rTransferAmount.add(_bonusAmount)
        );
